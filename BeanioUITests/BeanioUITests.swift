...
final class BeanioUITests: XCTestCase {
  ...
  func testTapppingUpdateLocationButton() throws {
    let app = XCUIApplication()
    app.launch()
    
    let updateLocationBarButton = app.buttons["updateLocationBarButton"]
    XCTAssert(updateLocationBarButton.exists)
    updateLocationBarButton.tap()
